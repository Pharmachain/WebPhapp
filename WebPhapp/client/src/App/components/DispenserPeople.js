import React, { Component } from "react";
import PropTypes from "prop-types";

class DispenserPeople extends Component {

  // Displays all the people in a table with clickable links to each patient.
  displayPeople = () => {
    return this.props.patientList.map(person => {
      return(
        <tr key={person.patientID}>
          <td>
            <a href = {"/dispenserPatient?ID=" + person.patientID}>
              {person.patientID}</a>
          </td>
          <td>
            <a href = {"/dispenserPatient?ID=" + person.patientID}>
              {person.first}
            </a>
          </td>
          <td>
            <a href = {"/dispenserPatient?ID=" + person.patientID}>
              {person.last}
            </a>
          </td>
          <td>
            <a href = {"/dispenserPatient?ID=" + person.patientID}>
              {person.dob}
            </a>
          </td>
        </tr>
      );
    });
  }

  render() {
    return (
      // Returns a table of patients styled according to the Argon style system
      <div className="card shadow">
        <div className="card-header border-0">
          <h3 className="mb-0">Dispenser's Patients</h3>
        </div>
        <div className="table-responsive">
          <table className="table align-items-center table-flush">
            <thead className="thead-light">
              <tr>
                <th scope="col">ID</th>
                <th scope="col">First Name</th>
                <th scope="col">Last Name</th>
                <th scope="col">Date Of Birth</th>
              </tr>
            </thead>
            <tbody>
              {this.displayPeople()}
            </tbody>
          </table>
        </div>
      </div>
    );
  }
}

// The properties of each person patient id, first name, last name, and data of birth
DispenserPeople.propTypes = {
  patientList: PropTypes.arrayOf(
      PropTypes.shape({
        patientID: PropTypes.number.isRequired,
        first: PropTypes.string,
        last: PropTypes.string,
        dob: PropTypes.string
      }).isRequired,
    )
}

export default DispenserPeople;

